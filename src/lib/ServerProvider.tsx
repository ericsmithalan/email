...
export function _makeProvider(contextTypes: object) {
    return class ProvideContext extends Component<Props> {
...
export function _render(self, ProvideContext) {
    ...
    const element = <ProvideContext context={self.context}>{self.props.children}</ProvideContext>;
...
function stackProviders(element, values) {
    for (let i = 0; i < values.length; i++) {
        ...
        element = <Context.Provider value={values[i]}>{childElement}</Context.Provider>;
    }
